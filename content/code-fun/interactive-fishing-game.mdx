---
slug: interactive-fishing-game
cover: cm0l1a31h000pg3ej22zaz3r7
title: 钓鱼咯🎣
description: 该页面展示了一个基于HTML5 Canvas技术实现的互动小游戏，用户可以通过点击屏幕钓取不同种类的鱼。这是一个学习和体验前端技术的优秀案例，特别适合那些对Canvas和JavaScript有兴趣的前端开发者。
keywords: 前端开发, HTML5 Canvas, JavaScript, 互动游戏, 鱼, 钓鱼小游戏, 前端技术, Canvas动画, Web编程, 网页互动
date: 2024-08-26
mobile: false
---
...
<html lang="zh">
  <head>
    ...
    <style>
      ...
      canvas {
        background: url('https://www.fecoder.cn/code-fun/images/oceanBackdrop.jpg')
          no-repeat top left;
  ...
  <body>
    <canvas>你的浏览器不支持 HTML5 Canvas</canvas>
    <script>
      ...
      (function () {
        ...
        var draw_fish = function (fish) {
          ...
          ctx.fillStyle = 'black';
          ctx.arc(
            fish.x + (fish.s + fish.s / 6) * fish.pos,
            fish.y,
            fish.s / 4,
            0,
            2 * Math.PI
        ...
        (function update() {
          ...
          ctx.fillText('数量: ' + fishCaught, 10, 10);
          ctx.fillText(
            [dig(Math.floor(secs / 60)), dig(secs % 60)].join(':'),
            10,
            30
          );
          for (let i in fish) {
