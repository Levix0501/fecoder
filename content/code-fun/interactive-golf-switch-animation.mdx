---
slug: interactive-golf-switch-animation
cover: cm0l1ahiv000qg3ej5nc49slw
title: 高尔夫switch
description: 本页面展示了一个使用前端技术制作的交互式高尔夫球开关动画效果。用户可以点击开关触发动画，模拟高尔夫球杆的击打动作，展示了如何通过CSS和JavaScript实现动态交互效果。
keywords: 前端技术, 交互设计, 高尔夫球动画, CSS, JavaScript, 动态效果, 网页动画, 用户体验
date: 2024-08-15
mobile: false
---
  ...
  <head>
    ...
    <style>
      ...
      .toggle {
        ...
        height: 10rem;
        border-radius: 5rem;
        background: linear-gradient(90deg, #80c0a1 -50%, #30d031 150%);
        cursor: pointer;
      ...
      toggleElement.addEventListener('click', () => {
        if (!toggleElement.classList.contains('toggle--no-events')) {
          ...
          if (!toggleElement.classList.contains('toggle--active')) {
            ...
            setTimeout(
              () => stickElement.classList.remove('stick--right'),
              1500
            );
          } else {
            stickElement.classList.add('stick--left');
            setTimeout(
              () => stickElement.classList.remove('stick--left'),
              1500
            );
          }
          setTimeout(
            () => toggleElement.classList.remove('toggle--no-events'),
            2000
          );
        }
      });
